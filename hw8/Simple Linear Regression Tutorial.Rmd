...
data = plant_gr)
sms <- plant_gr$soil.moisture.content
pgr <- plant_gr$plant.growth.rate

model_pgr <- lm(pgr ~ sms, data = plant_gr)
...
```
 ...
 Plant Growth = 12.75 * Soil Moisture + 19.35
...
```{r}
sms.new <- data.frame(seq(0,20,.401) / 10)

pred <- data.frame(predict(model_pgr, newdata = sms.new, interval = 'prediction'))

intervals <- data.frame(predict(model_pgr, newdata = sms.new, interval = 'confidence'))

all_data <- cbind(plant_gr, intervals)

ggplot(all_data, aes(x = sms,
y = pgr)) +
geom_point() +
geom_smooth(method = 'lm') +
geom_line(aes(y = pred$lwr), col = "coral2", linetype = "dashed") + 
geom_line(aes(y = pred$upr), col = "coral2", linetype = "dashed") +
ylab("Plant Growth Rate (mm/week)") +
xlab("Soil Moisture Content")
theme_bw()
